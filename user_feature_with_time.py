...
user_feature_with_time='jdata_sam/user_feature_with_time.csv'


#每个用户行为统计
def counter_type_cate_brand(group):
    ...
    group['cate4_num']=cate_cnt[4]
    group['cate5_num'] = cate_cnt[5]
    group['cate6_num'] = cate_cnt[6]
    group['cate7_num'] = cate_cnt[7]
    group['cate8_num'] = cate_cnt[8]
    group['cate9_num'] = cate_cnt[9]
    group['cate10_num'] = cate_cnt[10]
    group['cate11_num'] = cate_cnt[11]

    return group[['user_id', 'browse_num', 'addcart_num',
      'delcart_num', 'buy_num', 'favor_num',
      'click_num','cate4_num','cate5_num','cate6_num','cate7_num',
        'cate8_num','cate9_num','cate10_num','cate11_num']]
...
def statistic_count_action(fname,chunksize=100000):
    ...
    while loop:
        try:
            chunk=reader.get_chunk(chunksize)[['user_id','type','brand']]
            chunks.append(chunk)
    ...
    df_ac=df_ac.groupby(['user_id'],as_index=False).apply(counter_type_cate_brand)
...
if __name__=="__main__":
    ...
    all_user_feature.to_csv(user_feature_with_time,index=False)
